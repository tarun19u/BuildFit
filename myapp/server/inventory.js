...
const initializeDefaultStock = () => {
  return new Promise((resolve, reject) => {
    const defaultStock = [
      ...
      { id: 6, name: "Pre-Workout Energy", stock: 10 }, // Out of stock
...
const InventoryManager = {
  ...
  releaseStock: (productId, quantity) => {
    return new Promise((resolve, reject) => {
      inventoryDb.run(
        "UPDATE inventory SET reserved_quantity = MAX(0, reserved_quantity - ?), last_updated = CURRENT_TIMESTAMP WHERE product_id = ?",
        [quantity, productId],
        function(err) {
          if (err) {
            reject(err);
            ...
            // Log the release
            inventoryDb.run(
              "INSERT INTO stock_history (product_id, change_type, quantity_change, reason) VALUES (?, 'release', ?, 'Item removed from cart')",
              [productId, quantity]
            );
            resolve({ success: true, releasedQuantity: quantity });
          }
        }
      );
    });
  ...
  completePurchase: (items) => {
    return new Promise((resolve, reject) => {
      inventoryDb.serialize(() => {
        const results = [];
        let completed = 0;

        inventoryDb.run("BEGIN TRANSACTION");

        items.forEach(item => {
          const { productId, quantity } = item;

          inventoryDb.run(
            "UPDATE inventory SET stock_quantity = stock_quantity - ?, reserved_quantity = reserved_quantity - ?, last_updated = CURRENT_TIMESTAMP WHERE product_id = ?",
            [quantity, quantity, productId],
            function(err) {
              if (err) {
                inventoryDb.run("ROLLBACK");
                reject(err);
                return;
              }
              ...
              // Log the purchase
              inventoryDb.run(
                "INSERT INTO stock_history (product_id, change_type, quantity_change, reason) VALUES (?, 'purchase', ?, 'Item purchased')",
                [productId, -quantity]
              );
  ...
  // Update stock (admin function)
  updateStock: (productId, newQuantity, reason = 'Manual update') => {
    return new Promise((resolve, reject) => {
      inventoryDb.get(
        ...
        (err, row) => {
          ...
          const previousQuantity = row ? row.stock_quantity : 0;
          const quantityChange = newQuantity - previousQuantity;

          inventoryDb.run(
            "UPDATE inventory SET stock_quantity = ?, last_updated = CURRENT_TIMESTAMP WHERE product_id = ?",
            ...
            function(err) {
              if (err) {
                ...
                // Log the update
                inventoryDb.run(
                  "INSERT INTO stock_history (product_id, change_type, quantity_change, previous_quantity, new_quantity, reason) VALUES (?, 'restock', ?, ?, ?, ?)",
                  [productId, quantityChange, previousQuantity, newQuantity, reason]
                );
                resolve({ success: true, previousQuantity, newQuantity, quantityChange });
              }
            }
          );
        }
  ...
  getLowStockItems: () => {
    return new Promise((resolve, reject) => {
      inventoryDb.all(
        "SELECT * FROM inventory WHERE (stock_quantity - reserved_quantity) <= min_stock_level AND (stock_quantity - reserved_quantity) > 0",
        (err, rows) => {
          if (err) {
            reject(err);
          } else {
            resolve(rows.map(row => ({
              ...
              availableStock: row.stock_quantity - row.reserved_quantity,
  ...
  getStockHistory: (productId) => {
    return new Promise((resolve, reject) => {
      inventoryDb.all(
        "SELECT * FROM stock_history WHERE product_id = ? ORDER BY timestamp DESC LIMIT 50",
        [productId],
        (err, rows) => {
          if (err) {
            reject(err);
